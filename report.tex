...
\begin{figure}
\centering
\includegraphics[width=16cm]{LGCPCresults.png}
\caption{caption me}
...
The raw data shows an escalation in direct fire incidents towards the end of the period. Rather than a random fluctuation this may indicate a change in the underlying system dynamics, meaning that trying to fit a single set of hyper-parameters to the data is inappropriate. 
...
The simplest case is a step change in only one of the hyper parameters. REFERENCE considers a sudden drastic change in output scale of a gaussian process, which may be applicable to the SIGACTs data. 

This can be achieved by using the altered covariance function in \ref{eq:outputcpcov} where \(k_{se}\) is the squared exponential kernel with unity output scale and \(k_L\) is the correlation function from the previous model and \(a\) is defined in \ref{eq:outputcpcov2}. This function increases the output scales of both data series by a factor of \(c\) after some point \(x_c\).
